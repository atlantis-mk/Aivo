...
func (s *Store) SaveMCPDiagnostic(ctx context.Context, diagnostic domain.MCPDiagnostic) (domain.MCPDiagnostic, error) {
	if diagnostic.ID == "" {
		diagnostic.ID = uuid.NewString()
	}
	if diagnostic.Level == "" {
		diagnostic.Level = domain.MCPDiagnosticInfo
	}
	if diagnostic.TimeCreated == "" {
		diagnostic.TimeCreated = domain.NowString(time.Now())
	}
	row := pluginDiagnosticRow{
		ID: diagnostic.ID, ServerID: diagnostic.ServerID,
...
func (s *Store) ListMCPDiagnostics(ctx context.Context, serverID string, limit int) ([]domain.MCPDiagnostic, error) {
	...
	out := make([]domain.MCPDiagnostic, 0, len(rows))
	for _, row := range rows {
		out = append(out, mcpDiagnosticFromRow(row))
	}
	return out, nil
}
...
func (s *Store) SaveMCPServer(ctx context.Context, server domain.MCPServerConfig) (domain.MCPServerConfig, error) {
	...
	err := s.db.WithContext(ctx).Clauses(clause.OnConflict{
		...
		DoUpdates: clause.AssignmentColumns([]string{"name", "display_name", "description", "transport", "command", "args", "cwd", "env", "url", "headers", "auth_type", "bearer_token_env", "oauth_issuer_url", "oauth_client_id", "oauth_scopes", "oauth_access_token_ref", "oauth_refresh_token_ref", "oauth_expires_at", "roots", "timeout_seconds", "connect_timeout_seconds", "enabled", "status", "error", "time_updated"}),
	}).Create(&row).Error
	if err != nil {
...
func mcpDiagnosticFromRow(row pluginDiagnosticRow) domain.MCPDiagnostic {
	return domain.MCPDiagnostic{ID: row.ID, ServerID: row.ServerID, Level: row.Level, Message: row.Message, Metadata: decodeAnyMap(row.Metadata), TimeCreated: row.TimeCreated}
}
