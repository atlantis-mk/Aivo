...
func TestAgentRequestedToolReplacementChangesNextProviderSnapshot(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		switch index {
		case 1:
			_, _ = w.Write([]byte(`{"choices":[{"message":{"tool_calls":[{"id":"replace_tools","type":"function","function":{"name":"resource_resolve","arguments":"{\"mode\":\"use\",\"intent\":\"new capability\"}"}}]}}]}`))
		case 2:
			_, _ = w.Write([]byte(`{"choices":[{"message":{"content":"{\"resources\":[{\"kind\":\"tool\",\"id\":\"new_auto\"}]}"}}]}`))
		default:
	...
	mu.Unlock()
	if len(captured) != 3 {
		t.Fatalf("provider request count = %d, want primary, auxiliary replacement, and follow-up primary", len(captured))
	}
	...
	if !first["old_auto"] || first["new_auto"] || !first[projectQueryToolName] || !first[ResourceResolveName] {
		t.Fatalf("first Provider tools = %#v", first)
	}
	if len(auxiliary) != 0 {
		t.Fatalf("auxiliary replacement received executable tools: %#v", auxiliary)
	}
	if last["old_auto"] || !last["new_auto"] || !last[projectQueryToolName] || !last[ResourceResolveName] {
...
func TestResourceResolveReplacesAutomaticSetWithoutChangingManualSet(t *testing.T) {
	service, cleanup := newSessionTestService(t)
	defer cleanup()
	ctx := context.Background()
	session, err := service.CreateRuntimeSession(ctx, domain.CreateSessionRequest{Type: domain.SessionTypeCoding, ProjectPath: t.TempDir()})
	...
	for _, name := range []string{"manual_tool", "old_auto", "new_auto"} {
		description := name
		if name == "new_auto" {
			description = "new capability matched"
		}
		if err := registry.RegisterScoped(phase6EchoTool{spec: domain.ToolSpec{
			Name: name, Description: description, InputSchema: map[string]any{"type": "object"},
			Category: "extension", Toolsets: []string{"coding"}, ActivationPolicy: "auto",
		}}, domain.ToolSourceExtension, "replaceable", "v1"); err != nil {
			t.Fatal(err)
		}
	}
	if err := registry.RegisterScoped(NewResourceResolveTool(registry, service.resolveSessionResources), domain.ToolSourceBridge, "tool_selection", "v1"); err != nil {
		t.Fatal(err)
	}

	beforeActivations, beforeCandidates := service.snapshotToolCandidates(ctx, session.ID, "turn-1", registry, registry.Specs())
	...
	result := NewToolRuntime(registry, t.TempDir()).ExecuteWithContext(ctx, domain.ChatToolCall{
		ID: "replace", Name: ResourceResolveName, Arguments: json.RawMessage(`{"mode":"use","intent":"use the new capability"}`),
	...
	afterActivations, afterCandidates := service.snapshotToolCandidates(ctx, session.ID, "turn-1", registry, registry.Specs())
