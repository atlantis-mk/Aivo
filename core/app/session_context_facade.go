...
func (s *Service) CreateSummary(ctx context.Context, input domain.CreateSummaryRequest) (domain.SessionSummary, error) {
	...
	if summaryText == "" {
		summaryText = s.generatedSummary(ctx, input)
		if summaryText == "" {
			summaryText = s.fallbackSummary(ctx, input)
		}
	}
...
func (s *Service) CompactSessionContext(ctx context.Context, input domain.CompactSessionContextInput) (domain.CompactSessionContextResult, error) {
	return s.compactSessionContext(ctx, input, nil)
}

func (s *Service) compactSessionContext(ctx context.Context, input domain.CompactSessionContextInput, plan *compactionPlan) (domain.CompactSessionContextResult, error) {
	sessionID := strings.TrimSpace(input.SessionID)
	if sessionID == "" {
		return domain.CompactSessionContextResult{}, errors.New("sessionId is required")
	}
	currentState, _ := s.store.GetSessionExecutionState(ctx, sessionID)
	if currentState.Status == domain.ExecutionStatusRunning || currentState.Status == domain.ExecutionStatusCompacting {
		return domain.CompactSessionContextResult{}, errors.New("session context can only be compacted while idle")
	}
	eventsBefore, err := s.store.ListSessionEvents(ctx, sessionID, false, 500)
	if err != nil {
		return domain.CompactSessionContextResult{}, err
	}
	latest, _ := s.store.LatestSummary(ctx, sessionID)
	if plan == nil {
		start := firstEventAfterSummary(eventsBefore, latest)
		toIndex := len(eventsBefore) - 1
		for toIndex >= start && !compactableSessionEvent(eventsBefore[toIndex]) {
			toIndex--
		}
		if toIndex < start {
			return domain.CompactSessionContextResult{}, errors.New("no uncompacted conversation history is available")
		}
		fromEventID := ""
		if latest != nil {
			fromEventID = latest.ToEventID
		}
		plan = &compactionPlan{FromEventID: fromEventID, ToEventID: eventsBefore[toIndex].ID}
	}
	if strings.TrimSpace(plan.ToEventID) == "" {
		return domain.CompactSessionContextResult{}, errors.New("compaction boundary is required")
	}
	compactionKind := "manual"
	if input.Automatic {
		compactionKind = "automatic"
	}
	_, err = s.store.UpsertSessionExecutionState(ctx, domain.SessionExecutionState{SessionID: sessionID, Status: domain.ExecutionStatusCompacting, Reason: compactionKind + " compaction requested"})
	if err != nil {
		return domain.CompactSessionContextResult{}, err
	}
	summary, err := s.CreateSummary(ctx, domain.CreateSummaryRequest{SessionID: sessionID, FromEventID: plan.FromEventID, ToEventID: plan.ToEventID})
	if err != nil {
	...
	payload := map[string]any{
		"kind": "context_compacted", "mode": compactionKind, "summaryId": summary.ID,
		"sectionCount": len(contextResult.Sections), "fromEventId": plan.FromEventID, "toEventId": plan.ToEventID,
	}
	if input.Automatic {
		payload["thresholdPercent"] = plan.Pressure.ThresholdPercent
		payload["contextWindowTokens"] = plan.Pressure.ContextWindowTokens
		payload["capacitySource"] = plan.Pressure.CapacitySource
	}
