...
| `TOOL-INSPECTOR-DEDUPE-001` | `REQ-TOOL-002` | Consecutive identical invocation descriptions render once and missing descriptions render no title | `AT-TOOL-002` | Completed |
...
- The inspector flattens every group in the activity and orders all tool calls by `timeCreated`, shows a consecutive shared assistant invocation description only on its first applicable row, omits repeated or missing description titles, and keeps the tool name as supporting text with running, approval, success, or failure state shown through semantic shadcn variants.
