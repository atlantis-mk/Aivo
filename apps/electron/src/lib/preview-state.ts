...
const OPENAI_AUTH_ISSUER =
  (
    import.meta.env.VITE_AIVO_OPENAI_AUTH_ISSUER as string | undefined
  )?.trim() || "https://auth.openai.com";
const OPENAI_AUTH_URL =
  (
    import.meta.env.VITE_AIVO_OPENAI_BROWSER_AUTH_URL as string | undefined
  )?.trim() || `${OPENAI_AUTH_ISSUER}/oauth/authorize`;
const OPENAI_TOKEN_URL =
  (
    import.meta.env.VITE_AIVO_OPENAI_BROWSER_TOKEN_URL as string | undefined
  )?.trim() || `${OPENAI_AUTH_ISSUER}/oauth/token`;
const OPENAI_CLIENT_ID =
  (
    import.meta.env.VITE_AIVO_OPENAI_BROWSER_CLIENT_ID as string | undefined
  )?.trim() || "app_EMoamEEZ73f0CkXaXp7hrann";
const OPENAI_SCOPE =
  (
    import.meta.env.VITE_AIVO_OPENAI_BROWSER_SCOPES as string | undefined
  )?.trim() || "openid profile email offline_access";
...
function buildPreviewCatalog(state: PreviewState): CatalogState {
  ...
  for (const provider of catalog.providers) {
    const storedAccountIds =
      provider.id === "volcengine" ? volcengineProviderIds : [provider.id];
    const storedAccounts = storedAccountIds
      .flatMap((providerId) => normalizeStoredAuth(auth[providerId]))
      .sort(
        (first, second) =>
          Date.parse(second.connectedAt ?? "") -
          Date.parse(first.connectedAt ?? ""),
      );
    const displayAccounts = storedAccounts.filter(
      (account) => account.type !== "env",
    );
    const stored = storedAccounts[0];
    if (!stored) continue;
    if (provider.id === "volcengine") {
      const connectedProviderId = connectedVolcengineProviderId(auth);
      if (connectedProviderId && connectedProviderId !== provider.id) {
        provider.id = connectedProviderId;
        provider.name = providerDisplayName(provider.id);
        provider.baseUrl = providerBaseURLDefaults[provider.id];
    ...
    provider.accounts = displayAccounts.map((account) => ({
      id:
        account.id ||
        `${provider.id}:${account.type}:${account.connectedAt || account.accountId || account.environment || "account"}`,
      providerId: provider.id,
      method: account.type,
  ...
  catalog.providers = catalog.providers.map((provider) => {
    const isConnectedProvider =
      ...
      (provider.id === "volcengine" &&
        config.provider?.id &&
        volcengineProviderIds.includes(config.provider.id));
    if (isConnectedProvider && config.provider?.model) {
      provider.defaultModelId = config.provider.model;
    }
    return provider;
  });
  catalog.connectedProviders = catalog.providers.filter(
...
function connectedVolcengineProviderId(auth: PreviewState["auth"]) {
  return volcengineProviderIds
    .flatMap((providerId) =>
      normalizeStoredAuth(auth?.[providerId]).map((account) => ({
        providerId,
        connectedAt: account.connectedAt,
      })),
    )
...
function persistPreviewProvider(
  input: ProviderConnectInput,
  authType: string,
  authSecret?: PreviewStoredAuth,
) {
  ...
  config.provider = {
    ...
    type:
      input.type?.trim() ||
      (providerID === "openai" ? "openai" : "openai-compatible"),
...
export function updatePreviewModelPreferences(
  ...
  if (serviceTier) config.serviceTier = serviceTier;
  state.config = config;
  writePreviewState(state);
  return config;
}

export function updatePreviewPermissionMode(permissionMode: string) {
  const state = readPreviewState();
  const config = normalizePreviewConfig(state.config);
  config.defaultPermissionMode = permissionMode;
  state.config = config;
  writePreviewState(state);
  return config;
}

export function completePreviewInitialization(
  initialWorkspacePath: string,
  appName: string,
) {
  const state = readPreviewState();
...
export async function completePreviewOpenAIBrowserAuth(rawInput?: string) {
  ...
  body.set(
    "redirect_uri",
    pendingAuth.callbackUrl || `${window.location.origin}/setup`,
  );
  ...
  if (!response.ok) {
    ...
    throw new Error(
      errorText || `OpenAI token exchange failed (${response.status})`,
    );
  }
