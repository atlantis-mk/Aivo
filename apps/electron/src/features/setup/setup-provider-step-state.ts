...
export function useSetupProviderStepState({
  ...
  const [activeProvider, setActiveProvider] = useState<ProviderChoice | null>(
    null,
  );
  ...
  const refreshModelsForProvider = useCallback(
    async (
      ...
      if (
        !force &&
        !canRefreshProviderModels(provider, form, mode, key, catalog)
      ) {
        return;
      }
  ...
  const markOpenAIAuthorized = useCallback(
    (provider: ProviderChoice) => {
      ...
      setOauthStatus((current) => successfulOpenAIStatus(current, authMode));
  ...
  async function validateActiveProvider() {
    if (!activeProvider)
      return { completed: false } satisfies ProviderValidationResult;
  ...
  async function completeActiveProvider(submittedProvider?: ProviderChoice) {
    const provider = submittedProvider ?? activeProvider;
    if (!provider) return;
    if (
      isCustomProviderChoice(provider) &&
    ...
    const completed = await onContinue(
      ...
      authMode,
      apiKey,
      isCustomProviderChoice(provider) ? customProviderForm : undefined,
      effectiveSelectedModelId(),
    );
    if (completed) closeProvider();
  }

  async function submitActiveProvider(submittedProvider?: ProviderChoice) {
    ...
    if (
      ...
      if (providerValidated || oauthStatus?.status === "success") {
        await completeActiveProvider(submittedProvider);
        return;
      }
  ...
  useEffect(() => {
    if (!hasCodexDesktopBridge()) return;
    return window.aivoDesktop.codex.onLoginCompleted((completion) => {
      if (!completion.success || activeProvider?.id !== "openai") return;
      setOauthStarted(true);
      markOpenAIAuthorized(activeProvider);
    });
