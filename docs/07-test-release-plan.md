...
| Dynamic Agent modes | Built-in origin/reset, bounded CRUD, protected internal IDs, session/association referenced deletion and role-change refusal, management toolset omission, code/safe runtime capability restoration, bounded subagent allowlists, narrowed Provider delegation schema/context, execution revalidation, global/project precedence, restart persistence, schema-v6 creation, schema-v7 payload cleanup, schema-v8 association contract, picker integration, and responsive management states | `AT-AGENT-001`, `AT-SESSION-001`, `CT-RELIABILITY-001`, `AT-UI-001` evidence in the owning Work |
| Desktop static/build | Type safety, lint, renderer/main/preload build | `pnpm lint`, `pnpm build` |
| Provider management and diagnostics | Settings catalog/add/connect/confirmed-delete states, configured provider auth, and backend behavior | `AT-PROVIDER-001`, `AT-UI-001`, provider smoke command in `docs/provider-backend.md` |
