...
- The visible pre-call selection record starts with only a running status immediately before the auxiliary call. After Host validation, the same record contains bounded typed extension/MCP/tool/skill summaries plus request-only or conversation lifetime, or a generic failed status. MCP/extension members are represented only by source ID and count; exact concrete names remain in Host-owned session state and Tool Snapshots. Eligibility, snapshot identity, one-request lifetime, and call-time authorization remain the security controls. Candidate descriptions, schemas, auxiliary prompts/responses, credentials, endpoints, and authorization state are neither persisted in that record nor exposed to the renderer.
