...
export async function createPublication({ input, output, version, baseUrl, prefix = "aivo", publishedAt }) {
  ...
  if (!/^https:\/\//.test(normalizedBaseUrl)) throw new Error("R2 public base URL must use https");
  const timestamp = new Date(publishedAt);
  if (Number.isNaN(timestamp.valueOf())) throw new Error(`Invalid publication timestamp: ${publishedAt}`);
  ...
  for (const artifact of artifacts) {
    if (names.has(artifact.name)) throw new Error(`Duplicate normalized release artifact: ${artifact.name}`);
...
export async function validateReleaseSource({ root, version, tag }) {
  ...
  const desktopManifest = JSON.parse(await fs.readFile(path.join(root, "apps/electron/package.json"), "utf8"));
