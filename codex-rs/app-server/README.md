...
- Begin a turn: To send user input, call `turn/start` with the target `threadId` and the user's input. Optional fields let you override model, model provider, cwd, sandbox policy or experimental `permissions` profile selection, approval policy, approvals reviewer, etc. A provider override is retained by the thread and applies to subsequent turns, so the same conversation can move across connected providers. This immediately returns the new turn object. The app-server emits `turn/started` when that turn actually begins running.
...
- `turn/started` — `{ threadId, turn, model?, modelProvider? }` with the turn id, empty `items`, `status: "inProgress"`, and the turn's model source when known.
- `turn/completed` — `{ threadId, turn, model?, modelProvider? }` where `turn.status` is `completed`, `interrupted`, or `failed`; successful turns include their final agent message when available, and failures carry `{ error: { message, codexErrorInfo?, additionalDetails?, misalignment? } }`.
